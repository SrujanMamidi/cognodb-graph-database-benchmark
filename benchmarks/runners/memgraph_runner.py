...
def get_driver():
    return GraphDatabase.driver(MEMGRAPH_URI, auth=(MEMGRAPH_USERNAME, MEMGRAPH_PASSWORD))


def get_memgraph_query_plan(query_key: str = "Q5") -> Dict[str, Any]:
    driver = get_driver()
    plan_str = ""
    try:
        with driver.session() as session:
            res = session.run(f"EXPLAIN {QUERIES[query_key]}", id=1000).data()
            plan_str = str(res)
    ...
    return {
        ...
        "index_type": "Label+property index",
        "query_plan_evidence": plan_str,
    }


def run_memgraph_benchmark() -> Dict[str, Any]:
    driver = get_driver()
    ...
    for q_name in ["Q1", "Q2", "Q3", "Q4", "Q5", "Q6"]:
        ...
        print(f"Running Memgraph {q_name} (20 warm-up, 100 measured)...")
    ...
    print(f"Running Memgraph Q7 (Concurrent Mixed: {CONCURRENCY_WORKERS} workers, {TOTAL_Q7_OPERATIONS} ops, 80/20 ratio)...")
    ...
    # Query plan info
    results["Q5_plan"] = get_memgraph_query_plan("Q5")
    driver.close()
    return results
